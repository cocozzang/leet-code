import { findMedianSortedArraysOMN } from "./4-M+N-case";
...
describe("4번문제 test", () => {
  test("case1", () => {
    ...
    expect(findMedianSortedArraysOMN(nums1, nums2)).toBe(2);
  });
  ...
  test("case2", () => {
    ...
    expect(findMedianSortedArraysOMN(nums1, nums2)).toBe(2.5);
  });
  test("case3", () => {
    const nums1 = [1, 4, 5];
    const nums2 = [6, 8];

    expect(findMedianSortedArraysOMN(nums1, nums2)).toBe(5);
  });
  ...
  test("case4", () => {
    ...
    expect(findMedianSortedArraysOMN(nums1, nums2)).toBe(240);
  });
  ...
  test("시간복잡도 O(log(m+n)) 검증", () => {
    ...
    const measureExecutionTime = (nums1: number[], nums2: number[]) => {
      ...
      findMedianSortedArraysOMN(nums1, nums2);
